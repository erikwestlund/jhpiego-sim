...
In this document I walk through my simulation of ANC. All the parameters here can be tweaked. I've made judgments about what I think work best. 

# Simulation Parameters

## Population ANC in Kenya, Nigeria, India, and Senegal

This is drawn from the provided spreadsheet.
...
simulate_preeclampsia_outcomes <- function(
  ...
  results <- data.frame(
    ...
    effect_size = numeric(n_simulations),
    population_size = rep(population_size, n_simulations),
    p_reached = numeric(n_simulations),
    n_reached = integer(n_simulations),
    p_uptake = numeric(n_simulations),
    n_uptake = integer(n_simulations),
    p_adverse_event = numeric(n_simulations),
    n_no_intervention = integer(n_simulations),
    adverse_events_no_intervention = integer(n_simulations),
    adjusted_adverse_event_rate = numeric(n_simulations),
    adverse_events_with_intervention = integer(n_simulations),
    total_adverse_events = integer(n_simulations),
    cases_prevented = numeric(n_simulations)
  )

  for (sim in 1:n_simulations) {
    if (sim %% 10000 == 0) {
      cat("Iteration:", format(sim, scientific = FALSE), "of", format(n_simulations, scientific = FALSE), "\n")
      flush.console()
    }

    # Simulate proportion of people reached
    p_reached <- rbeta(1, alpha_reached, beta_reached)
    n_reached <- round(population_size * p_reached)

    # Simulate intervention uptake among those reached
    p_uptake <- rbeta(1, alpha_uptake, beta_uptake)
    n_uptake <- rbinom(1, n_reached, p_uptake)

    # Simulate adverse events without intervention
    p_adverse_event <- rbeta(1, alpha_adverse_event, beta_adverse_event)
    n_no_intervention <- population_size - n_uptake
    adverse_events_no_intervention <- rbinom(1, n_no_intervention, p_adverse_event)

    # Draw effect size from uniform distribution
    effect_size <- runif(1, effect_size_min, effect_size_max)
    adjusted_adverse_event_rate <- p_adverse_event * (1 - effect_size)

    # Simulate adverse events with intervention (applying effect size reduction)
    adverse_events_with_intervention <- rbinom(1, n_uptake, adjusted_adverse_event_rate)

    # Total cases and cases prevented
    ...
    # Store results in pre-allocated data frame
    results$effect_size[sim] <- effect_size
    results$p_reached[sim] <- p_reached
    results$n_reached[sim] <- n_reached
    results$p_uptake[sim] <- p_uptake
    results$n_uptake[sim] <- n_uptake
    results$p_adverse_event[sim] <- p_adverse_event
    results$n_no_intervention[sim] <- n_no_intervention
    results$adverse_events_no_intervention[sim] <- adverse_events_no_intervention
    results$adjusted_adverse_event_rate[sim] <- adjusted_adverse_event_rate
    results$adverse_events_with_intervention[sim] <- adverse_events_with_intervention
    results$total_adverse_events[sim] <- total_adverse_events
    results$cases_prevented[sim] <- cases_prevented
  }
